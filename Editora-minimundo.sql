...
-- CRIAÇÃO DAS TABELAS SECUNDÁRIAS E RELACIONAMENTOS
CREATE TABLE LIVRO (
...
-- Adicionar índice no nome do AUTOR
ALTER TABLE AUTOR
ADD INDEX idx_nome_autor (nome);

-- Aumentar o tamanho da coluna 'codigo' em AREA_CONHECIMENTO
ALTER TABLE AREA_CONHECIMENTO
MODIFY COLUMN codigo VARCHAR(20);

-- Tornar a coluna 'descricao' da tabela DEPARTAMENTO obrigatória
ALTER TABLE DEPARTAMENTO
MODIFY COLUMN descricao TEXT NOT NULL;

-- Adicionar coluna de "estoque_minimo" na tabela LIVRO
ALTER TABLE LIVRO
ADD COLUMN estoque_minimo INT DEFAULT 5;


-- Adicionar coluna "ativo" à tabela FUNCIONARIO
ALTER TABLE FUNCIONARIO
ADD COLUMN ativo BOOLEAN DEFAULT TRUE;

-- Adicionar uma nova coluna "frete" à tabela PEDIDO
ALTER TABLE PEDIDO
ADD COLUMN frete DECIMAL(10,2) DEFAULT 0;

-- Alterar o tipo da coluna 'descricao' em PALAVRA_CHAVE para TEXT
ALTER TABLE PALAVRA_CHAVE
MODIFY COLUMN descricao TEXT NOT NULL;

-- Adicionar coluna "data_alteracao" à tabela CLIENTE para controle de atualização de dados
ALTER TABLE CLIENTE
ADD COLUMN data_alteracao DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP;

-- Adicionar chave estrangeira da editora ao departamento (hipotética relação organizacional)
ALTER TABLE DEPARTAMENTO
ADD COLUMN id_editora INT,
ADD FOREIGN KEY (id_editora) REFERENCES EDITORA(id_editora);

ALTER TABLE FUNCIONARIO ADD ativo BOOLEAN DEFAULT TRUE;

ALTER TABLE ITEM_PEDIDO MODIFY quantidade INT;
...
DELETE FROM LIVRO_PALAVRA WHERE isbn = '9780000000009';
